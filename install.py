#!/usr/bin/env python3
"""
Cross-platform installation script for STAC Explorer QGIS Plugin.
...
def install_plugin(source_dir, plugin_dir, plugin_name=PLUGIN_NAME):
...
def remove_plugin(plugin_dir, plugin_name=PLUGIN_NAME):
...
def main():
    """Run the install/remove script."""
    parser = argparse.ArgumentParser(
        description="Install or remove STAC Explorer Plugin"
    )
    parser.add_argument(
        "--remove",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default=PLUGIN_NAME,
        help=f"Plugin folder name in QGIS plugins directory (default: {PLUGIN_NAME})",
    )
